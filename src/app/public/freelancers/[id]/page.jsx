...
import { getReviewByFreelancerId } from "@/lib/api/review";
...
const FreelancerDetails = async ({ params }) => {
  ...
  const reviews = await getReviewByFreelancerId(id);

  const totalReviews = reviews.length;

  const totalLikes = reviews.reduce(
    (sum, review) => sum + (review.like || 0),
    0
  );
  ...
  return (<div className="max-w-5xl mx-auto px-4 py-10">
    {/* Cover */} <div className="h-24 rounded-3xl bg-gradient-to-r from-cyan-500 via-sky-500 to-blue-600"></div>
